<template>
  <div style="padding:5px;">
    ...
    <Button v-if="isadmin==2" style="margin-right: 4px;" @click="showCmdForm" type="primary" :disabled="cmdStatus.execing > 0">{{ $t('Execute command') }}</Button>
  ...
  methods: {
    ...
    handleSearch() {
      this.filteredDevices = this.devlists.filter((d) => {
        ...
        return d.id.toLowerCase().indexOf(filterString) > -1 || d.company.toLowerCase().indexOf(filterString) > -1 || d.username.toLowerCase().indexOf(filterString) > -1;
      });
    },
    getDevices() {
